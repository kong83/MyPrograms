...
namespace SurgeryHelper.Tools
{
    public class CDatabasesMerger
    {
        ...
        private string GetInfoMessageAboutCard(CHospitalizationWorker hospitalizationWorker, CVisitWorker visitWorker, int hospitalizationId, int visitId)
        {
            string infoStr;
            if (hospitalizationId == -1)
            {
                CVisit visit = visitWorker.GetById(visitId);
                infoStr = "консультации за '" + CConvertEngine.DateTimeToString(visit.VisitDate, true) + "'";
            }
            else
            {
                CHospitalization hospitalization = hospitalizationWorker.GetById(hospitalizationId);
                infoStr = "госпитализации за '" + CConvertEngine.DateTimeToString(hospitalization.DeliveryDate, true) + "'";
            }
        ...
        /// <param name="ownPatientId">id пациента в нашей базе</param>
        /// <param name="foreignPatientId">id пациента во внешней базе</param>
            ...
            int foreignVisitId)
        {
            ...
            if (isOwnObjectExists && !isForeignObjectExists)
            {
                ...
                {
                    IdOwnPatient = ownPatientId,
                    IdForeignPatient = foreignPatientId,
            ...
            foreach (CardType cardType in cardTypes)
            {
                ...
                if (isOwnObjectExists && !isForeignObjectExists)
                {
                    paientMergeInfo = new CMergeInfo
                    {
                        ...
                        TypeOfCard = cardType,
                        Difference = string.Format(
                            "Пациент: '{0}'. Нозология:'{1}'. {2}. Название объекта: '{3}'",
                            patientFIO,
                            patientNosology,
                            dateHospitalizationOrVisitInfoStr,
                            CCard.GetDescription(cardType))
                    };
        ...
        private static string GetDifferenceBetweenContentOfTwoPrivateFolders(string folderPathOwn, string folderPathForeign, string patientFIO, string patientNosology)
        {
            ...
            CObjectsByMergeAdder.GetFilesPathsFromAllFolders(folderPathOwn, ownFilePathsList);

            var foreignFilePathsList = new List<string>();
            CObjectsByMergeAdder.GetFilesPathsFromAllFolders(folderPathForeign, foreignFilePathsList);

            // Сравниваем файлы нашего пациента с файлами пациента из внешней базы 
            foreach (string ownFullPath in ownFilePathsList)
            {
                string ownRelativePath = CObjectsByMergeAdder.GetRelativePath(folderPathOwn, ownFullPath);

                string foreignFullPathSave = string.Empty;
                foreach (string foreignFullPath in foreignFilePathsList)
                {
                    if (ownRelativePath.ToLower() == CObjectsByMergeAdder.GetRelativePath(folderPathForeign, foreignFullPath).ToLower())
                    {
            ...
            foreach (string foreignFullPath in foreignFilePathsList)
            {
                string foreignRelativePath = CObjectsByMergeAdder.GetRelativePath(folderPathForeign, foreignFullPath);

                string ownFullPathSave = string.Empty;
                foreach (string ownFullPath in ownFilePathsList)
                {
                    if (foreignRelativePath.ToLower() == CObjectsByMergeAdder.GetRelativePath(folderPathOwn, ownFullPath).ToLower())
                    {
        ...
        /// <param name="copyPrivateData">Надо ли копировать данные из личной папки</param>
        public void AddPatientToOwnDatabase(int foreignPatientId, bool copyPrivateData)
        {
            ...
            CObjectsByMergeAdder.AddPatient(
                _mergeForm, "own", _ownNosologyWorker, _foreignWorkersKeeper, _ownWorkersKeeper, foreignPatient,
                ...
                _foreignRangeOfMotionCardWorker, _ownRangeOfMotionCardWorker, _foreignCardWorker, _ownCardWorker, copyPrivateData);
        ...
        /// <param name="copyPrivateData">Надо ли копировать данные из личной папки</param>
        public void AddPatientToForeignDatabase(int ownPatientId, bool copyPrivateData)
        {
            ...
            CObjectsByMergeAdder.AddPatient(
                _mergeForm, "foreign", _foreignNosologyWorker, _ownWorkersKeeper, _foreignWorkersKeeper, ownPatient,
                ...
                _ownRangeOfMotionCardWorker, _foreignRangeOfMotionCardWorker, _ownCardWorker, _foreignCardWorker, copyPrivateData);
            _mergeForm.SetStatus("\r\nДанные о пациенте '" + ownPatient.GetFullName() + "' с нозологией '" + ownPatient.Nosology + "' успешно добавлены во внешнюю базу");
        }
        #endregion


        #region Удаление пациентов из нашей и внешней баз
        /// <summary>
        /// Удалить пациента из нашей базы
        /// </summary>
        /// <param name="ownPatientId">id пациента из нашей базы, которого надо удалить</param>
        public void RemovePatientFromOwnDatabase(int ownPatientId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить пациента '{0}' с нозологией '{1}' из нашей базы? Данная операция необратима.", ownPatient.GetFullName(), ownPatient.Nosology), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;                
            }

            _mergeForm.SetStatus(string.Format("Удаление пациента '{0}' с нозологией '{1}' из нашей базы.", ownPatient.GetFullName(), ownPatient.Nosology));
            _ownWorkersKeeper.PatientWorker.Remove(ownPatientId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }

        /// <summary>
        /// Удалить пациента из внешней базы
        /// </summary>
        /// <param name="foreignPatientId">id пациента из внешней базы, которого надо удалить</param>
        public void RemovePatientFromForeignDatabase(int foreignPatientId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить пациента '{0}' с нозологией '{1}' из внешней базы? Данная операция необратима.", foreignPatient.GetFullName(), foreignPatient.Nosology), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление пациента '{0}' с нозологией '{1}' из внешней базы.", foreignPatient.GetFullName(), foreignPatient.Nosology));
            _foreignWorkersKeeper.PatientWorker.Remove(foreignPatientId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
        ...
        public void AddAnamneseToOwnDatabase(int foreignPatientId, int ownPatientId)
        {
            ...
            CObjectsByMergeAdder.AddAnamnese(
        ...
        #region Удаление анамнезов из нашей и внешней баз
        /// <summary>
        /// Удалить анамнез пациента из нашей базы
        /// </summary>
        /// <param name="ownPatientId">id пациента из нашей базы, у которого надо удалить анамнез</param>
        public void RemoveAnamneseFromOwnDatabase(int ownPatientId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить анамнез пациента '{0}' с нозологией '{1}' из нашей базы? Данная операция необратима.", ownPatient.GetFullName(), ownPatient.Nosology), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление анамнеза пациента '{0}' с нозологией '{1}' из нашей базы.", ownPatient.GetFullName(), ownPatient.Nosology));
            _ownWorkersKeeper.AnamneseWorker.Remove(ownPatientId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }

        /// <summary>
        /// Удалить анамнез пациента из внешней базы
        /// </summary>
        /// <param name="foreignPatientId">id пациента из внешней базы, у которого надо удалить анамнез</param>
        public void RemoveAnamneseHistoryFromForeignDatabase(int foreignPatientId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить анамнез пациента '{0}' с нозологией '{1}' из внешней базы? Данная операция необратима.", foreignPatient.GetFullName(), foreignPatient.Nosology), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление анамнеза пациента '{0}' с нозологией '{1}' из внешней базы.", foreignPatient.GetFullName(), foreignPatient.Nosology));
            _foreignWorkersKeeper.AnamneseWorker.Remove(foreignPatientId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
        ...
        public void AddObstetricHistoryToOwnDatabase(int foreignPatientId, int ownPatientId)
        {
            ...
            CObjectsByMergeAdder.AddObstetricHistory(
        ...
        #region Удаление акушерских анамнезов из нашей и внешней баз
        /// <summary>
        /// Удалить акушерский анамнез пациента из нашей базы
        /// </summary>
        /// <param name="ownPatientId">id пациента из нашей базы,  у которого надо удалить акушерский анамнез</param>
        public void RemoveObstetricHistoryFromOwnDatabase(int ownPatientId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить акушерский анамнез пациента '{0}' с нозологией '{1}' из нашей базы? Данная операция необратима.", ownPatient.GetFullName(), ownPatient.Nosology), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление акушерского анамнеза пациента '{0}' с нозологией '{1}' из нашей базы.", ownPatient.GetFullName(), ownPatient.Nosology));
            _ownWorkersKeeper.ObstetricHistoryWorker.Remove(ownPatientId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }

        /// <summary>
        /// Удалить акушерский анамнез пациента из внешней базы
        /// </summary>
        /// <param name="foreignPatientId">id пациента из внешней базы, у которого надо удалить акушерский анамнез</param>
        public void RemoveObstetricHistoryFromForeignDatabase(int foreignPatientId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить акушерский анамнез пациента '{0}' с нозологией '{1}' из внешней базы? Данная операция необратима.", foreignPatient.GetFullName(), foreignPatient.Nosology), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление акушерского анамнеза пациента '{0}' с нозологией '{1}' из внешней базы.", foreignPatient.GetFullName(), foreignPatient.Nosology));
            _foreignWorkersKeeper.ObstetricHistoryWorker.Remove(foreignPatientId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
            ...
            int foreignPatientId, int ownPatientId, int foreignHospitalizationId)
        {
            ...
            CObjectsByMergeAdder.AddHospitalization(
        ...
        #region Удаление госпитализаций из нашей и внешней баз
        /// <summary>
        /// Удалить госпитализацию из нашей базы
        /// </summary>        
        /// <param name="ownPatientId">id пациента из нашей базы, у которого надо удалить госпитализацию</param>
        /// <param name="ownHospitalizationId">id госпитализации из нашей базы, которую надо удалить</param>
        public void RemoveHospitalizationFromOwnDatabase(int ownPatientId, int ownHospitalizationId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);
            CHospitalization ownHospitalization = _ownHospitalizationWorker.GetById(ownHospitalizationId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить госпитализацию за '{0}' у пациента '{1}' из нашей базы? Данная операция необратима.",
                CConvertEngine.DateTimeToString(ownHospitalization.DeliveryDate, true), ownPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление госпитализации за '{0}' у пациента '{1}' из нашей базы.",
                CConvertEngine.DateTimeToString(ownHospitalization.DeliveryDate, true), ownPatient.GetFullName()));
            _ownHospitalizationWorker.Remove(ownHospitalizationId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }


        /// <summary>
        /// Удалить госпитализацию из внешней базы
        /// </summary>        
        /// <param name="foreignPatientId">id пациента из внешней базы, у которого надо удалить госпитализацию</param>
        /// <param name="foreignHospitalizationId">id госпитализации из внешней базы, которую надо удалить</param>
        public void RemoveHospitalizationFromForeignDatabase(int foreignPatientId, int foreignHospitalizationId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);
            CHospitalization foreignHospitalization = _foreignHospitalizationWorker.GetById(foreignHospitalizationId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить госпитализацию за '{0}' у пациента '{1}' из внешней базы? Данная операция необратима.",
                CConvertEngine.DateTimeToString(foreignHospitalization.DeliveryDate, true), foreignPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление госпитализации за '{0}' у пациента '{1}' из внешней базы.",
                CConvertEngine.DateTimeToString(foreignHospitalization.DeliveryDate, true), foreignPatient.GetFullName()));
            _foreignHospitalizationWorker.Remove(foreignHospitalizationId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
            ...
            int foreignHospitalizationId, int ownHospitalizationId)
        {
            ...
            CObjectsByMergeAdder.AddDischargeEpicrisis(
                _foreignDischargeEpicrisisWorker,
                _ownDischargeEpicrisisWorker,
                foreignHospitalizationId,
                ownHospitalizationId);
        ...
        #region Удаление выписных эпикризов из нашей и внешней баз
        /// <summary>
        /// Удалить выписной эпикриз из нашей базы
        /// </summary>        
        /// <param name="ownPatientId">id пациента из нашей базы</param>
        /// <param name="ownHospitalizationId">id госпитализации из нашей базы, у которой надо удалить выписной эпикриз</param>
        public void RemoveDischargeEpicrisisFromOwnDatabase(int ownPatientId, int ownHospitalizationId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);
            CHospitalization ownHospitalization = _ownHospitalizationWorker.GetById(ownHospitalizationId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить выписной эпикриз у госпитализации за '{0}' у пациента '{1}' из нашей базы? Данная операция необратима.",
                CConvertEngine.DateTimeToString(ownHospitalization.DeliveryDate, true), ownPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление выписного эпикриза у госпитализации за '{0}' у пациента '{1}' из нашей базы.",
                CConvertEngine.DateTimeToString(ownHospitalization.DeliveryDate, true), ownPatient.GetFullName()));
            _ownDischargeEpicrisisWorker.Remove(ownHospitalizationId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }


        /// <summary>
        /// Удалить выписной эпикриз из внешней базы
        /// </summary>        
        /// <param name="foreignPatientId">id пациента из внешней базы</param>
        /// <param name="foreignHospitalizationId">id госпитализации из внешней базы, у которой надо удалить выписной эпикриз</param>
        public void RemoveDischargeEpicrisisFromForeignDatabase(int foreignPatientId, int foreignHospitalizationId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);
            CHospitalization foreignHospitalization = _foreignHospitalizationWorker.GetById(foreignHospitalizationId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить выписной эпикриз у госпитализации за '{0}' у пациента '{1}' из внешней базы? Данная операция необратима.",
                CConvertEngine.DateTimeToString(foreignHospitalization.DeliveryDate, true), foreignPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление выписного эпикриза у госпитализации за '{0}' у пациента '{1}' из внешней базы.",
                CConvertEngine.DateTimeToString(foreignHospitalization.DeliveryDate, true), foreignPatient.GetFullName()));
            _foreignDischargeEpicrisisWorker.Remove(foreignHospitalizationId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
            ...
            int foreignHospitalizationId, int ownHospitalizationId)
        {
            ...
            CObjectsByMergeAdder.AddLineOfCommunicationEpicrisis(
                _foreignLineOfCommunicationEpicrisisWorker,
                _ownLineOfCommunicationEpicrisisWorker,
                foreignHospitalizationId,
                ownHospitalizationId);
        ...
        #region Удаление этапных эпикризов из нашей и внешней баз
        /// <summary>
        /// Удалить этапный эпикриз из нашей базы
        /// </summary>        
        /// <param name="ownPatientId">id пациента из нашей базы</param>
        /// <param name="ownHospitalizationId">id госпитализации из нашей базы, у которой надо удалить этапный эпикриз</param>
        public void RemoveLineOfCommunicationEpicrisisFromOwnDatabase(int ownPatientId, int ownHospitalizationId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);
            CHospitalization ownHospitalization = _ownHospitalizationWorker.GetById(ownHospitalizationId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить этапный эпикриз у госпитализации за '{0}' у пациента '{1}' из нашей базы? Данная операция необратима.",
                CConvertEngine.DateTimeToString(ownHospitalization.DeliveryDate, true), ownPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление этапного эпикриза у госпитализации за '{0}' у пациента '{1}' из нашей базы.",
                CConvertEngine.DateTimeToString(ownHospitalization.DeliveryDate, true), ownPatient.GetFullName()));
            _ownLineOfCommunicationEpicrisisWorker.Remove(ownHospitalizationId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }


        /// <summary>
        /// Удалить этапный эпикриз из внешней базы
        /// </summary>        
        /// <param name="foreignPatientId">id пациента из внешней базы</param>
        /// <param name="foreignHospitalizationId">id госпитализации из внешней базы, у которой надо удалить этапный эпикриз</param>
        public void RemoveLineOfCommunicationEpicrisisFromForeignDatabase(int foreignPatientId, int foreignHospitalizationId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);
            CHospitalization foreignHospitalization = _foreignHospitalizationWorker.GetById(foreignHospitalizationId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить этапный эпикриз у госпитализации за '{0}' у пациента '{1}' из внешней базы? Данная операция необратима.",
                CConvertEngine.DateTimeToString(foreignHospitalization.DeliveryDate, true), foreignPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление этапного эпикриза у госпитализации за '{0}' у пациента '{1}' из внешней базы.",
                CConvertEngine.DateTimeToString(foreignHospitalization.DeliveryDate, true), foreignPatient.GetFullName()));
            _foreignLineOfCommunicationEpicrisisWorker.Remove(foreignHospitalizationId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
            ...
            int foreignHospitalizationId, int ownHospitalizationId)
        {
            ...
            CObjectsByMergeAdder.AddTransferableEpicrisis(
                _foreignTransferableEpicrisisWorker,
                _ownTransferableEpicrisisWorker,
                foreignHospitalizationId,
                ownHospitalizationId);
        ...
        #region Удаление переводных эпикризов из нашей и внешней баз
        /// <summary>
        /// Удалить переводной эпикриз из нашей базы
        /// </summary>        
        /// <param name="ownPatientId">id пациента из нашей базы</param>
        /// <param name="ownHospitalizationId">id госпитализации из нашей базы, у которой надо удалить переводной эпикриз</param>
        public void RemoveTransferableEpicrisisFromOwnDatabase(int ownPatientId, int ownHospitalizationId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);
            CHospitalization ownHospitalization = _ownHospitalizationWorker.GetById(ownHospitalizationId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить переводной эпикриз у госпитализации за '{0}' у пациента '{1}' из нашей базы? Данная операция необратима.",
                CConvertEngine.DateTimeToString(ownHospitalization.DeliveryDate, true), ownPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление переводного эпикриза у госпитализации за '{0}' у пациента '{1}' из нашей базы.",
                CConvertEngine.DateTimeToString(ownHospitalization.DeliveryDate, true), ownPatient.GetFullName()));
            _ownTransferableEpicrisisWorker.Remove(ownHospitalizationId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }


        /// <summary>
        /// Удалить переводной эпикриз из внешней базы
        /// </summary>        
        /// <param name="foreignPatientId">id пациента из внешней базы</param>
        /// <param name="foreignHospitalizationId">id госпитализации из внешней базы, у которой надо удалить переводной эпикриз</param>
        public void RemoveTransferableEpicrisisFromForeignDatabase(int foreignPatientId, int foreignHospitalizationId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);
            CHospitalization foreignHospitalization = _foreignHospitalizationWorker.GetById(foreignHospitalizationId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить переводной эпикриз у госпитализации за '{0}' у пациента '{1}' из внешней базы? Данная операция необратима.",
                CConvertEngine.DateTimeToString(foreignHospitalization.DeliveryDate, true), foreignPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление переводного эпикриза у госпитализации за '{0}' у пациента '{1}' из внешней базы.",
                CConvertEngine.DateTimeToString(foreignHospitalization.DeliveryDate, true), foreignPatient.GetFullName()));
            _foreignTransferableEpicrisisWorker.Remove(foreignHospitalizationId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
            ...
            int foreignHospitalizationId, int ownHospitalizationId)
        {
            ...
            CObjectsByMergeAdder.AddMedicalInspection(
                _foreignMedicalInspectionWorker,
                _ownMedicalInspectionWorker,
                foreignHospitalizationId,
                ownHospitalizationId);
        ...
        #region Удаление осмотров в отделении из нашей и внешней баз
        /// <summary>
        /// Удалить осмотр в отделении из нашей базы
        /// </summary>        
        /// <param name="ownPatientId">id пациента из нашей базы</param>
        /// <param name="ownHospitalizationId">id госпитализации из нашей базы, у которой надо удалить осмотр в отделении</param>
        public void RemoveMedicalInspectionFromOwnDatabase(int ownPatientId, int ownHospitalizationId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);
            CHospitalization ownHospitalization = _ownHospitalizationWorker.GetById(ownHospitalizationId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить осмотр в отделении у госпитализации за '{0}' у пациента '{1}' из нашей базы? Данная операция необратима.",
                CConvertEngine.DateTimeToString(ownHospitalization.DeliveryDate, true), ownPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление осмотра в отделении у госпитализации за '{0}' у пациента '{1}' из нашей базы.",
                CConvertEngine.DateTimeToString(ownHospitalization.DeliveryDate, true), ownPatient.GetFullName()));
            _ownMedicalInspectionWorker.Remove(ownHospitalizationId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }


        /// <summary>
        /// Удалить осмотр в отделении из внешней базы
        /// </summary>        
        /// <param name="foreignPatientId">id пациента из внешней базы</param>
        /// <param name="foreignHospitalizationId">id госпитализации из внешней базы, у которой надо удалить осмотр в отделении</param>
        public void RemoveMedicalInspectionFromForeignDatabase(int foreignPatientId, int foreignHospitalizationId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);
            CHospitalization foreignHospitalization = _foreignHospitalizationWorker.GetById(foreignHospitalizationId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить осмотр в отделении у госпитализации за '{0}' у пациента '{1}' из внешней базы? Данная операция необратима.",
                CConvertEngine.DateTimeToString(foreignHospitalization.DeliveryDate, true), foreignPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление осмотра в отделении у госпитализации за '{0}' у пациента '{1}' из внешней базы.",
                CConvertEngine.DateTimeToString(foreignHospitalization.DeliveryDate, true), foreignPatient.GetFullName()));
            _foreignMedicalInspectionWorker.Remove(foreignHospitalizationId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
            ...
            int foreignHospitalizationId, int ownHospitalizationId, int foreignOperationId)
        {
            ...
            CObjectsByMergeAdder.AddOperationAndProtocol(
                _ownHospitalizationWorker,
                _foreignOperationWorker,
                _ownOperationWorker,
                _foreignOperationProtocolWorker,
        ...
        #region Удаление операций (и их протоколов) из нашей и внешней баз
        /// <summary>
        /// Удалить операцию и протокол из нашей базы
        /// </summary>        
        /// <param name="ownPatientId">id пациента из нашей базы</param>
        /// <param name="ownHospitalizationId">id госпитализации из нашей базы, у которой надо удалить операцию и протокол</param>
        /// <param name="ownOperationId">id операции, которую надо удалить</param>
        public void RemoveOperationAndProtocolFromOwnDatabase(int ownPatientId, int ownHospitalizationId, int ownOperationId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);
            CHospitalization ownHospitalization = _ownHospitalizationWorker.GetById(ownHospitalizationId);
            COperation ownOperation = _ownOperationWorker.GetById(ownOperationId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить операцию '{0}' у госпитализации за '{1}' у пациента '{2}' из нашей базы? Данная операция необратима.",
                ownOperation.Name, CConvertEngine.DateTimeToString(ownHospitalization.DeliveryDate, true), ownPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление операции '{0}' у госпитализации за '{1}' у пациента '{2}' из нашей базы.",
                ownOperation.Name, CConvertEngine.DateTimeToString(ownHospitalization.DeliveryDate, true), ownPatient.GetFullName()));
            _ownOperationWorker.Remove(ownOperationId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }


        /// <summary>
        /// Удалить операцию и протокол из внешней базы
        /// </summary>        
        /// <param name="foreignPatientId">id пациента из внешней базы</param>
        /// <param name="foreignHospitalizationId">id госпитализации из внешней базы, у которой надо удалить операцию и протокол</param>
        /// <param name="foreignOperationId">id операции, которую надо удалить</param>
        public void RemoveOperationAndProtocolFromForeignDatabase(int foreignPatientId, int foreignHospitalizationId, int foreignOperationId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);
            CHospitalization foreignHospitalization = _foreignHospitalizationWorker.GetById(foreignHospitalizationId);
            COperation foreignOperation = _foreignOperationWorker.GetById(foreignOperationId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить операцию '{0}' у госпитализации за '{1}' у пациента '{2}' из внешней базы? Данная операция необратима.",
                foreignOperation.Name, CConvertEngine.DateTimeToString(foreignHospitalization.DeliveryDate, true), foreignPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление операции '{0}' у госпитализации за '{1}' у пациента '{2}' из внешней базы.",
                foreignOperation.Name, CConvertEngine.DateTimeToString(foreignHospitalization.DeliveryDate, true), foreignPatient.GetFullName()));
            _foreignOperationWorker.Remove(foreignOperationId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
        ...
        #region Удаление консультации из нашей и внешней баз
        /// <summary>
        /// Удалить консультацию из нашей базы
        /// </summary>        
        /// <param name="ownPatientId">id пациента из нашей базы, у которого надо удалить консультацию</param>
        /// <param name="ownVisitId">id консультации из нашей базы, который надо удалить</param>
        public void RemoveVisitFromOwnDatabase(int ownPatientId, int ownVisitId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);
            CVisit ownVisit = _ownVisitWorker.GetById(ownVisitId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить консультацию за '{0}' у пациента '{1}' из нашей базы? Данная операция необратима.",
                CConvertEngine.DateTimeToString(ownVisit.VisitDate, true), ownPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление консультации за '{0}' у пациента '{1}' из нашей базы.",
                CConvertEngine.DateTimeToString(ownVisit.VisitDate, true), ownPatient.GetFullName()));
            _ownVisitWorker.Remove(ownVisitId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }

        /// <summary>
        /// Удалить консультацию из внешней базы
        /// </summary>        
        /// <param name="foreignPatientId">id пациента из внешней базы, у которого надо удалить консультацию</param>
        /// <param name="foreignVisitId">id консультации из внешней базы, который надо удалить</param>
        public void RemoveVisitFromForeignDatabase(int foreignPatientId, int foreignVisitId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);
            CVisit foreignVisit = _foreignVisitWorker.GetById(foreignVisitId);

            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить консультацию за '{0}' у пациента '{1}' из внешней базы? Данная операция необратима.",
                CConvertEngine.DateTimeToString(foreignVisit.VisitDate, true), foreignPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление консультации за '{0}' у пациента '{1}' из внешней базы.",
                CConvertEngine.DateTimeToString(foreignVisit.VisitDate, true), foreignPatient.GetFullName()));
            _foreignVisitWorker.Remove(foreignVisitId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
        ...
        public void AddBrachialPlexusCardToOwnDatabase(
            ...
            int ownVisitId)
        {
            string infoStr = GetInfoMessageAboutCard(_foreignHospitalizationWorker, _foreignVisitWorker, foreignHospitalizationId, foreignVisitId);

            _mergeForm.SetStatus(string.Format(
                "Добавление данных о карте на плечевое сплетение для {0} в нашу базу.", infoStr));

            CObjectsByMergeAdder.AddBrachialPlexusCard(
                _foreignBrachialPlexusCardWorker,
                _ownBrachialPlexusCardWorker,
                foreignHospitalizationId,
                ownHospitalizationId,
        ...
        public void AddBrachialPlexusCardToForeignDatabase(
            ...
            int foreignVisitId)
        {
            string infoStr = GetInfoMessageAboutCard(_ownHospitalizationWorker, _ownVisitWorker, ownHospitalizationId, ownVisitId);
        ...
        #region Удаление карты на плечевое сплетение из нашей и внешней баз
        /// <summary>
        /// Удалить карту на плечевое сплетение из нашей базы
        /// </summary>        
        /// <param name="ownPatientId">id пациента из нашей базы</param>
        /// <param name="ownHospitalizationId">id госпитализации из нашей базы, у которой надо удалить карту на плечевое сплетение</param>
        /// <param name="ownVisitId">id консультации из нашей базы, у которого надо удалить карту на плечевое сплетение</param>
        public void RemoveBrachialPlexusCardFromOwnDatabase(int ownPatientId, int ownHospitalizationId, int ownVisitId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);
            ...
            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить карту на плечевое сплетение у {0} у пациента '{1}' из нашей базы? Данная операция необратима.",
                infoStr, ownPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление карты на плечевое сплетение {0} у пациента '{1}' из нашей базы.", infoStr, ownPatient.GetFullName()));
            _ownBrachialPlexusCardWorker.Remove(ownHospitalizationId, ownVisitId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }

        /// <summary>
        /// Удалить карту на плечевое сплетение из внешней базы
        /// </summary>        
        /// <param name="foreignPatientId">id пациента из внешней базы</param>
        /// <param name="foreignHospitalizationId">id госпитализации из внешней базы, у которой надо удалить карту на плечевое сплетение</param>
        /// <param name="foreignVisitId">id консультации из внешней базы, у которого надо удалить карту на плечевое сплетение</param>
        public void RemoveBrachialPlexusCardFromForeignDatabase(int foreignPatientId, int foreignHospitalizationId, int foreignVisitId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);
            ...
            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить карту на плечевое сплетение у {0} у пациента '{1}' из внешней базы? Данная операция необратима.",
                infoStr, foreignPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление карты на плечевое сплетение {0} у пациента '{1}' из внешней базы.", infoStr, foreignPatient.GetFullName()));
            _foreignBrachialPlexusCardWorker.Remove(foreignHospitalizationId, foreignVisitId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
            ...
            int ownVisitId)
        {
            ...
            CObjectsByMergeAdder.AddObstetricParalysisCard(
                _foreignObstetricParalysisCardWorker,
                _ownObstetricParalysisCardWorker,
                foreignHospitalizationId,
                ownHospitalizationId,
        ...
        #region Удаление карты на акушерский паралич из нашей и внешней баз
        /// <summary>
        /// Удалить карту на акушерский паралич из нашей базы
        /// </summary>        
        /// <param name="ownPatientId">id пациента из нашей базы</param>
        /// <param name="ownHospitalizationId">id госпитализации из нашей базы, у которой надо удалить карту на акушерский паралич</param>
        /// <param name="ownVisitId">id консультации из нашей базы, у которого надо удалить карту на акушерский паралич</param>
        public void RemoveObstetricParalysisCardFromOwnDatabase(int ownPatientId, int ownHospitalizationId, int ownVisitId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);
            ...
            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить карту на акушерский паралич у {0} у пациента '{1}' из нашей базы? Данная операция необратима.",
                infoStr, ownPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление карты на акушерский паралич {0} у пациента '{1}' из нашей базы.", infoStr, ownPatient.GetFullName()));
            _ownObstetricParalysisCardWorker.Remove(ownHospitalizationId, ownVisitId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }

        /// <summary>
        /// Удалить карту на акушерский паралич из внешней базы
        /// </summary>        
        /// <param name="foreignPatientId">id пациента из внешней базы</param>
        /// <param name="foreignHospitalizationId">id госпитализации из внешней базы, у которой надо удалить карту на акушерский паралич</param>
        /// <param name="foreignVisitId">id консультации из внешней базы, у которого надо удалить карту на акушерский паралич</param>
        public void RemoveObstetricParalysisCardFromForeignDatabase(int foreignPatientId, int foreignHospitalizationId, int foreignVisitId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);
            ...
            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить карту на акушерский паралич у {0} у пациента '{1}' из внешней базы? Данная операция необратима.",
                infoStr, foreignPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление карты на акушерский паралич {0} у пациента '{1}' из внешней базы.", infoStr, foreignPatient.GetFullName()));
            _foreignObstetricParalysisCardWorker.Remove(foreignHospitalizationId, foreignVisitId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
            ...
            int ownVisitId)
        {
            ...
            CObjectsByMergeAdder.AddRangeOfMotionCard(
                _foreignRangeOfMotionCardWorker,
                _ownRangeOfMotionCardWorker,
                foreignHospitalizationId,
                ownHospitalizationId,
        ...
        #region Удаление карты на объём движений из нашей и внешней баз
        /// <summary>
        /// Удалить карту на объём движений из нашей базы
        /// </summary>        
        /// <param name="ownPatientId">id пациента из нашей базы</param>
        /// <param name="ownHospitalizationId">id госпитализации из нашей базы, у которой надо удалить карту на объём движений</param>
        /// <param name="ownVisitId">id консультации из нашей базы, у которого надо удалить карту на объём движений</param>
        public void RemoveRangeOfMotionCardFromOwnDatabase(int ownPatientId, int ownHospitalizationId, int ownVisitId)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);
            ...
            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить карту на объём движений у {0} у пациента '{1}' из нашей базы? Данная операция необратима.",
                infoStr, ownPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление карты на объём движений {0} у пациента '{1}' из нашей базы.", infoStr, ownPatient.GetFullName()));
            _ownRangeOfMotionCardWorker.Remove(ownHospitalizationId, ownVisitId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }

        /// <summary>
        /// Удалить карту на объём движений из внешней базы
        /// </summary>        
        /// <param name="foreignPatientId">id пациента из внешней базы</param>
        /// <param name="foreignHospitalizationId">id госпитализации из внешней базы, у которой надо удалить карту на объём движений</param>
        /// <param name="foreignVisitId">id консультации из внешней базы, у которого надо удалить карту на объём движений</param>
        public void RemoveRangeOfMotionCardFromForeignDatabase(int foreignPatientId, int foreignHospitalizationId, int foreignVisitId)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);
            ...
            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить карту на объём движений у {0} у пациента '{1}' из внешней базы? Данная операция необратима.",
                infoStr, foreignPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление карты на объём движений {0} у пациента '{1}' из внешней базы.", infoStr, foreignPatient.GetFullName()));
            _foreignRangeOfMotionCardWorker.Remove(foreignHospitalizationId, foreignVisitId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
            ...
            CardType cardType)
        {
            ...
            CObjectsByMergeAdder.AddLeftRightCard(
                _foreignCardWorker,
                _ownCardWorker,
                cardType,
                foreignHospitalizationId,
        ...
        #region Удаление карты с двумя частями из нашей и внешней баз
        /// <summary>
        /// Удалить карту с двумя частями из нашей базы
        /// </summary>        
        /// <param name="ownPatientId">id пациента из нашей базы</param>
        /// <param name="ownHospitalizationId">id госпитализации из нашей базы, у которой надо удалить карту с двумя частями</param>
        /// <param name="ownVisitId">id консультации из нашей базы, у которого надо удалить карту с двумя частями</param>
        /// <param name="cardType">Тип карты</param>
        public void RemoveLeftRightCardFromOwnDatabase(int ownPatientId, int ownHospitalizationId, int ownVisitId, CardType cardType)
        {
            CPatient ownPatient = _ownPatientWorker.GetById(ownPatientId);
            ...
            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить карту '{0}' у {1} у пациента '{2}' из нашей базы? Данная операция необратима.",
                CCard.GetDescription(cardType), infoStr, ownPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление карты '{0}' {1} у пациента '{2}' из нашей базы.", CCard.GetDescription(cardType), infoStr, ownPatient.GetFullName()));
            _ownCardWorker.Remove(ownHospitalizationId, ownVisitId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }

        /// <summary>
        /// Удалить карту с двумя частями из внешней базы
        /// </summary>        
        /// <param name="foreignPatientId">id пациента из внешней базы</param>
        /// <param name="foreignHospitalizationId">id госпитализации из внешней базы, у которой надо удалить карту с двумя частями</param>
        /// <param name="foreignVisitId">id консультации из внешней базы, у которого надо удалить карту с двумя частями</param>
        /// <param name="cardType">Тип карты</param>
        public void RemoveLeftRightCardFromForeignDatabase(int foreignPatientId, int foreignHospitalizationId, int foreignVisitId, CardType cardType)
        {
            CPatient foreignPatient = _foreignPatientWorker.GetById(foreignPatientId);
            ...
            if (DialogResult.No == MessageBox.ShowDialog(
                string.Format("Вы уверены, что хотите удалить карту '{0}' у {1} у пациента '{2}' из внешней базы? Данная операция необратима.",
                CCard.GetDescription(cardType), infoStr, foreignPatient.GetFullName()), "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                return;
            }

            _mergeForm.SetStatus(string.Format("Удаление карты '{0}' {1} у пациента '{2}' из внешней базы.", CCard.GetDescription(cardType), infoStr, foreignPatient.GetFullName()));
            _foreignCardWorker.Remove(foreignHospitalizationId, foreignVisitId);
            _mergeForm.SetStatus("\r\nДанные успешно удалены");
        }
        #endregion
        ...
        private void ChangePatient(ObjectType typeOfObject, CPatient patient, string value, string messagePart)
        {
            switch (typeOfObject)
            {
                ...
                case ObjectType.PatientIsSpecifyLegalRepresent:
                    patient.IsSpecifyLegalRepresent = value == "Указан";
                    _mergeForm.SetStatus("Информация о том, указан ли законный представитель" + messagePart);
                    break;
                case ObjectType.PatientLegalRepresent:
                    patient.LegalRepresent = value;
                    _mergeForm.SetStatus("Информация о законном представителе" + messagePart);
                    break;
                case ObjectType.PatientWorkPlace:
                    patient.WorkPlace = value;
                    _mergeForm.SetStatus("Место работы" + messagePart);
                    break;                    
        ...
        public void ChangePatientParameterInOwnDatabase(ObjectType typeOfObject, int ownPatientId, string value)
        {
        ...
        public void ChangePatientParameterInForeignDatabase(ObjectType typeOfObject, int foreignPatientId, string value)
        {
        ...
        public void ChangeAnamneseParameterInOwnDatabase(ObjectType typeOfObject, int ownPatientId, string value)
        {
            CPatient patient = _ownPatientWorker.GetById(ownPatientId);
            string messagePart = string.Format(
        ...
        public void ChangeAnamneseParameterInForeignDatabase(ObjectType typeOfObject, int foreignPatientId, string value)
        {
            CPatient patient = _foreignPatientWorker.GetById(foreignPatientId);
            string messagePart = string.Format(
        ...
        public void ChangeObstetricHistoryParameterInOwnDatabase(ObjectType typeOfObject, int ownPatientId, object value)
        {
            CPatient patient = _ownPatientWorker.GetById(ownPatientId);
            string messagePart = string.Format(
        ...
        public void ChangeObstetricHistoryParameterInForeignDatabase(ObjectType typeOfObject, int foreignPatientId, object value)
        {
            CPatient patient = _foreignPatientWorker.GetById(foreignPatientId);
            string messagePart = string.Format(
        ...
        public void ChangeHospitalizationParameterInOwnDatabase(ObjectType typeOfObject, int ownPatientId, int ownHospitalizationId, string value)
        {
            CPatient patient = _ownPatientWorker.GetById(ownPatientId);
        ...
        public void ChangeHospitalizationParameterInForeignDatabase(ObjectType typeOfObject, int foreignPatientId, int foreignHospitalizationId, string value)
        {
            CPatient patient = _foreignPatientWorker.GetById(foreignPatientId);
        ...
        private void ChangeDischargeEpicrisis(ObjectType typeOfObject, CDischargeEpicrisis dischargeEpicrisis, string value, string messagePart)
        {
            switch (typeOfObject)
            {
                case ObjectType.DischargeEpicrisisAdditionalAnalises:
                    ...
                    break;
                case ObjectType.DischargeEpicrisisAnalysisDate:
                    if (value == "Нет значения")
                    {
                        dischargeEpicrisis.AnalysisDate = null;
                    }
                    else
                    {
                        dischargeEpicrisis.AnalysisDate = CConvertEngine.StringToDateTime(value);
                    }

                    _mergeForm.SetStatus("Время взятия анализов" + messagePart);
                    break;
        ...
        public void ChangeDischargeEpicrisisParameterInOwnDatabase(ObjectType typeOfObject, int ownHospitalizationId, object value)
        {
            CHospitalization hospitalization = _ownHospitalizationWorker.GetById(ownHospitalizationId);
        ...
        public void ChangeDischargeEpicrisisParameterInForeignDatabase(ObjectType typeOfObject, int foreignHospitalizationId, object value)
        {
            CHospitalization hospitalization = _foreignHospitalizationWorker.GetById(foreignHospitalizationId);
        ...
        public void ChangeLineOfCommunicationEpicrisisParameterInOwnDatabase(ObjectType typeOfObject, int ownHospitalizationId, string value)
        {
            CHospitalization hospitalization = _ownHospitalizationWorker.GetById(ownHospitalizationId);
            CPatient patient = _ownPatientWorker.GetById(hospitalization.PatientId);
            string messagePart = string.Format(
        ...
        public void ChangeLineOfCommunicationEpicrisisParameterInForeignDatabase(ObjectType typeOfObject, int foreignHospitalizationId, string value)
        {
            CHospitalization hospitalization = _foreignHospitalizationWorker.GetById(foreignHospitalizationId);
            CPatient patient = _foreignPatientWorker.GetById(hospitalization.PatientId);
            string messagePart = string.Format(
        ...
        public void ChangeTransferableEpicrisisParameterInOwnDatabase(ObjectType typeOfObject, int ownHospitalizationId, string value)
        {
            CHospitalization hospitalization = _ownHospitalizationWorker.GetById(ownHospitalizationId);
            CPatient patient = _ownPatientWorker.GetById(hospitalization.PatientId);
            string messagePart = string.Format(
        ...
        public void ChangeTransferableEpicrisisParameterInForeignDatabase(ObjectType typeOfObject, int foreignHospitalizationId, string value)
        {
            CHospitalization hospitalization = _foreignHospitalizationWorker.GetById(foreignHospitalizationId);
            CPatient patient = _foreignPatientWorker.GetById(hospitalization.PatientId);
            string messagePart = string.Format(
        ...
        public void ChangeMedicalInspectionParameterInOwnDatabase(ObjectType typeOfObject, int ownHospitalizationId, object value)
        {
            CHospitalization hospitalization = _ownHospitalizationWorker.GetById(ownHospitalizationId);
            CPatient patient = _ownPatientWorker.GetById(hospitalization.PatientId);
            string messagePart = string.Format(
        ...
        public void ChangeMedicalInspectionParameterInForeignDatabase(ObjectType typeOfObject, int foreignHospitalizationId, object value)
        {
            CHospitalization hospitalization = _foreignHospitalizationWorker.GetById(foreignHospitalizationId);
            CPatient patient = _foreignPatientWorker.GetById(hospitalization.PatientId);
            string messagePart = string.Format(
        ...
        #region Изменение параметров у консультации
        ...
        public void ChangeVisitParameterInOwnDatabase(ObjectType typeOfObject, int ownPatientId, int ownVisitId, string value)
        {
            CPatient patient = _ownPatientWorker.GetById(ownPatientId);
        ...
        public void ChangeVisitParameterInForeignDatabase(ObjectType typeOfObject, int foreignPatientId, int foreignVisitId, string value)
        {
            CPatient patient = _foreignPatientWorker.GetById(foreignPatientId);
        ...
        public void ChangeBrachialPlexusCardInOwnDatabase(ObjectType typeOfObject, int ownHospitalizationId, int ownVisitId, object value)
        {
            ...
            if (ownHospitalizationId > -1)
            {
                ...
                dateOfHospitalizationOrVisitInfo = "госпитализации за '" + CConvertEngine.DateTimeToString(hospitalization.DeliveryDate, true) + "'";
            }
            else
            {
                CVisit visit = _ownVisitWorker.GetById(ownVisitId);
                patientId = visit.PatientId;
                dateOfHospitalizationOrVisitInfo = "консультации за '" + CConvertEngine.DateTimeToString(visit.VisitDate, true) + "'";
            }
        ...
        public void ChangeLeftRightCardInOwnDatabase(ObjectType typeOfObject, int ownHospitalizationId, int ownVisitId, CardSide ownSideOfCard, CardType ownTypeOfCard, object value)
        {
            ...
            CPatient patient = _ownPatientWorker.GetById(patientId);
            string messagePart = string.Format(
                " для {0} {1} сторона для {2} для пациента '{3}' с нозологией '{4}' успешно изменена в нашей базе.",
                CCard.GetDescription(ownTypeOfCard),
                CCard.GetDescription(ownSideOfCard),
                dateOfHospitalizationOrVisitInfo,
                patient.GetFullName(),
                patient.Nosology);
        ...
        public void ChangeLeftRightCardInForeignDatabase(ObjectType typeOfObject, int foreignHospitalizationId, int foreignVisitId, CardSide foreignSideOfCard, CardType foreignTypeOfCard, object value)
        {
            ...
            CPatient patient = _foreignPatientWorker.GetById(patientId);
            string messagePart = string.Format(
                " для {0} {1} сторона для {2} для пациента '{3}' с нозологией '{4}' успешно изменена во внешней базе.",
                CCard.GetDescription(foreignTypeOfCard),
                CCard.GetDescription(foreignSideOfCard),
                dateOfHospitalizationOrVisitInfo,
                patient.GetFullName(),
                patient.Nosology);
