...
namespace UpdateDatabase
{
    class Program
    {
        ...
        {
            const string help =
                "usage:  updatedatabase.exe <command name>\r\n" +
                "Commands:\r\n" +
                "  generatebackup\r\n" +
        ...
        {
            Trace.TraceInformation("Execute external script {0} for the database {1}...\r\n",
            ...
            using (var sqlScriptFile = new StreamReader(externalUpdateScriptPath))
            {
                ...
                if (string.Compare(databaseName, Configuration.Default.DefaultDatabaseName, true) == 0)
                {
                    externalSqlScript = externalSqlScript.Replace(
                        "$__IS_DEFAULT_DATABASE__",
                        "1");
                }
                else
                {
